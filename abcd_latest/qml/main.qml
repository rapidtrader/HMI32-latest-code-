...
ApplicationWindow {
    ...
    readonly property var pageSources: ({
        ...
        "A25Sensor": "A25Sensor.qml",
        "WaterPressurePage": "WaterPressurePage.qml"
    })
    ...
    Connections {
        ...
        onErrorOccurred: function (message, severity) {
            console.log("main.qml: onErrorOccurred called with message:", message, "severity:", severity)
    ...
    Connections {
        ...
        onCommandErrorOccurred: function (message, severity) {
            console.log("main.qml: onCommandErrorOccurred called with message:", message, "severity:", severity)
            addErrorPopup(message, severity)
    ...
    Component {
        ...
        Rectangle {
            ...
            property string severityLevel: model.severityLevel
            property int itemIndex: model.itemIndex
            ...
            RowLayout {
                ...
                Button {
                    ...
                    onClicked: {
                        console.log("Removing popup at index:", popup.itemIndex)
                        if (popup.itemIndex >= 0 && popup.itemIndex < errorPopups.count) {
            ...
            Component.onCompleted: {
                console.log("errorPopupComponent: Component.onCompleted, messageText:", JSON.stringify(popup.messageText), "severityLevel:", JSON.stringify(popup.severityLevel))
                opacity = 1
    ...
    function addErrorPopup(message, severity) {
        console.log("main.qml: addErrorPopup called with message:", JSON.stringify(message), "severity:", JSON.stringify(severity))
        errorPopups.append({
            "messageText": message,
            "severityLevel": severity,
            "itemIndex": errorPopups.count
        })
        console.log("main.qml: errorPopups now has count:", errorPopups.count)
        console.log("main.qml: last item messageText:", errorPopups.get(errorPopups.count - 1).messageText)
    ...
    function addToNotificationHistory(message, severity) {
        console.log("main.qml: addToNotificationHistory called")
        // This will be called from bridge Python side, but for now we'll trigger it manually
        // after error popup is added
    }
 ...
 Connections {
    ...
    source: pageSources[appController.currentPage]

    Connections {
        target: appController
        onPageChanged: {
            pageLoader.source = pageSources[appController.currentPage]
        }
    }

    onStatusChanged: {
        console.log("[PAGE LOADER] status =", status, "source =", source)

        if (status === Loader.Null) {
            console.log("[PAGE LOADER] NULL")
        } else if (status === Loader.Ready) {
            console.log("[PAGE LOADER] READY:", source)
        } else if (status === Loader.Loading) {
            console.log("[PAGE LOADER] LOADING:", source)
        } else if (status === Loader.Error) {
            console.log("[PAGE LOADER] ERROR:", source)
        }
    }
